...
const CreateJobPage: React.FC<CreateJobPageProps> = ({ isDark, user, jobType, onBack }) => {
  ...
  const handleMediaUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setJobData(prev => ({ 
      ...
      // Wenn es das erste Bild ist, setze es als Titelbild
  ...
  const removeMedia = (index: number) => {
    setJobData(prev => {
      ...
      // Titelbild-Index anpassen wenn nötig
      let newTitleIndex = prev.titleImageIndex;
      if (index === prev.titleImageIndex && newMedia.length > 0) {
        newTitleIndex = 0; // Erstes verfügbares Bild wird Titelbild
        ...
        return (
          <div className="space-y-6">
            ...
            <div>
              ...
              {/* Medien Vorschau mit Titelbild-Auswahl */}
              {jobData.media.length > 0 && (
                <div className="mt-6">
                  ...
                  {/* Titelbild */}
                  {jobData.media.filter(file => file.type.startsWith('image/')).length > 0 && (
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {jobData.media
                          ...
                          <div
                            key={originalIndex}
                            onClick={() => setTitleImage(originalIndex)}
                            className={`relative aspect-video rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${
                              jobData.titleImageIndex === originalIndex
                          ...
                          >
                            ...
                            <div className="absolute bottom-1 left-1 bg-black/70 text-white text-xs px-2 py-1 rounded">
                              {jobData.titleImageIndex === originalIndex ? 'Titelbild' : 'Als Titel wählen'}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div>
                    <span className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      Alle Dateien ({jobData.media.length})
                    </span>
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-2 text-xs text-gray-500">
                                ...
                                {index === jobData.titleImageIndex && file.type.startsWith('image/') && (
                                  <span className="text-blue-500 font-medium">• Titelbild</span>
                                )}
                              </div>
                            </div>
                          </div>
            ...
            <div className={`${isDark ? 'bg-slate-700/50 border-slate-600' : 'bg-gray-50 border-gray-200'} rounded-2xl p-6 border`}>
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                    {selectedJobType === 'cash' 
                      ? 'Zusätzliche Karma Punkte anbieten' 
                      : 'Zusätzliches Geld anbieten'
                    }
                  </p>
                </div>
                  ...
                  {selectedJobType === 'cash' ? (
                    <div>
                      ...
                      <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        Extra Karma für besonders gute Arbeit
                      </p>
                    </div>
                  ) : (
                    <div>
                      ...
                      <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        Kleine Geld-Belohnung zusätzlich zum Karma
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={isDark ? 'text-white' : 'text-gray-900'}>
                    {jobData.media.length} Dateien
                    {jobData.media.filter(f => f.type.startsWith('image/')).length > 0 && ' (mit Titelbild)'}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Hauptbezahlung:</span>
